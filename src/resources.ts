...
export type ImageResourceConfig = Required<
  Pick<
    ResourceKeyValues,
    ResourceKey.FORMAT | ResourceKey.WIDTH | ResourceKey.HEIGHT
  >
>;

export type AndroidAdaptiveIconConfig = ImageResourceConfig &
  Required<
    Pick<
      ResourceKeyValues,
      ResourceKey.FOREGROUND | ResourceKey.BACKGROUND | ResourceKey.DENSITY
    >
  >;

export type AndroidIconConfig = ImageResourceConfig &
  Required<Pick<ResourceKeyValues, ResourceKey.SRC | ResourceKey.DENSITY>>;

export type AndroidSplashConfig = ImageResourceConfig &
  Required<
    Pick<
      ResourceKeyValues,
      | ResourceKey.SRC
    ...
    >
  >;
export type IOSIconConfig = ImageResourceConfig &
  Required<Pick<ResourceKeyValues, ResourceKey.SRC>>;

export type IOSSplashConfig = ImageResourceConfig &
  Required<Pick<ResourceKeyValues, ResourceKey.SRC | ResourceKey.ORIENTATION>>;

export type WindowsIconConfig = Required<
  Pick<
    ResourceKeyValues,
    | ResourceKey.SRC
    | ResourceKey.FORMAT
...
export type WindowsSplashConfig = Required<
  Pick<
    ResourceKeyValues,
    | ResourceKey.SRC
    | ResourceKey.FORMAT
...
): ResourcesTypeConfig<AndroidAdaptiveIconConfig>;

export function getResourcesConfig(
  platform: Platform.ANDROID | Platform.IOS | Platform.WINDOWS,
...
export function getResourcesConfig(
  platform: Platform,
  type: ResourceType,
): ResourcesTypeConfig<ResourceKeyValues> {
  switch (platform) {
...
export function generateScaledWindowsResource<
  T extends WindowsIconConfig | WindowsSplashConfig
...
const ANDROID_ADAPTIVE_ICON_RESOURCES_CONFIG: ResourcesTypeConfig<AndroidAdaptiveIconConfig> = {
  ...
  configXml: {
    nodeName: 'icon',
    nodeAttributes: [
      NodeAttributes.FOREGROUND,
      NodeAttributes.DENSITY,
      NodeAttributes.BACKGROUND,
    ],
    xpaths: resource => [
      `icon[@foreground][@background][@density='${resource.density}']`,
    ],
    included: resource =>
      ANDROID_ADAPTIVE_ICON_RESOURCES.map(r => `${r.density}`).includes(
        `${resource.density}`,
      ),
  },
};
...
const IOS_SPLASH_568H_2X_IPHONE = {
...
const IOS_SPLASH_2688H_LANDSCAPE_IPHONE = {
...
const IOS_SPLASH_1792H_LANDSCAPE_IPHONE = {
...
const IOS_SPLASH_2436H_LANDSCAPE = {
...
const IOS_SPLASH_736H_LANDSCAPE = {
...
const IOS_SPLASH_LANDSCAPE_2X_IPAD = {
...
const IOS_SPLASH_LANDSCAPE_IPADPRO = {
...
const IOS_SPLASH_LANDSCAPE_IPAD = {
...
const IOS_SPLASH_PORTRAIT_2X_IPAD = {
...
const IOS_SPLASH_PORTRAIT_IPADPRO = {
...
const IOS_SPLASH_PORTRAIT_IPAD = {
...
const IOS_SPLASH_2X_UNIVERSAL_ANYANY = {
